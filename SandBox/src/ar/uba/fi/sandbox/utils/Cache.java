...
public class Cache {
	...
	public static Object getCachedObject(String key){
		String id =  toSHA1(key.getBytes());
			try {
				ObjectInputStream fis = new ObjectInputStream(SandboxApp.getContext().openFileInput(id + ".json"));
	...
	public static void saveTo(String key, Object o){
		String id =  toSHA1(key.getBytes());
		try{
			SandboxApp.getContext().deleteFile(id);
			FileOutputStream fos = 	SandboxApp.getContext().openFileOutput(id + ".json", Context.MODE_PRIVATE);
